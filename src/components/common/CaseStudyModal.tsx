...
export const CaseStudyModal: React.FC<CaseStudyModalProps> = ({ project, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#1C1C1C]/60 backdrop-blur-xs overflow-y-auto animate-in fade-in duration-200">
      <div className="relative w-full max-w-3xl bg-[#FAFAF8] border border-[#1C1C1C]/30 shadow-2xl overflow-hidden my-8 text-[#1C1C1C] flex flex-col max-h-[90vh]">
        {/* Top Header */}
        ...
        {/* Content */}
        <div className="p-6 sm:p-6 overflow-y-auto space-y-6">
          <div>
            <h2 className="text-3xl sm:text-4xl font-sans tracking-tight text-[#1C1C1C]">{project.title}</h2>
          ...
          <div>
            <h3 className="text-[10px] font-sans font-bold uppercase tracking-[0.25em] text-[#1C1C1C] flex items-center gap-2 mb-3">
              <Layers className="w-3.5 h-3.5 text-[#C7A600]" />
            ...
            <div className="space-y-2.5">
              {project.approach.map((step, idx) => (
                <div key={idx} className="flex items-start gap-3 p-3 bg-white border border-[#1C1C1C]/10">
                  <div className="w-5 h-5 bg-[#1C1C1C] text-[#FAFAF8] flex items-center justify-center font-sans font-bold text-[10px] shrink-0 mt-0.5">
                    {idx + 1}
                  </div>
          ...
          <div className="p-4 bg-white border border-[#1C1C1C]/15 shadow-sm">
            <h3 className="text-[10px] font-sans font-bold uppercase tracking-[0.25em] text-[#C7A600] flex items-center gap-1.5 mb-2.5">
            ...
            <ul className="space-y-2">
              {project.impact.map((imp, idx) => (
                <li key={idx} className="flex items-start gap-2 text-xs font-serif text-[#1C1C1C]/85">
                  <span className="text-[#C7A600] font-bold">✓</span>
                  <span>{imp}</span>
                </li>
              ))}
            </ul>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {project.technologies.map((t, idx) => (
                <span key={idx} className="text-xs font-sans px-2.5 py-1 bg-[#FFF8DA] text-[#1C1C1C] border border-[#1C1C1C]/10">
                  {t}
                </span>
              ))}
            </div>
        ...
        {/* Footer actions */}
        <div className="px-6 py-4 border-t border-[#1C1C1C]/15 bg-[#FFF8DA] flex flex-wrap items-center justify-between gap-3">
          <div>
            {project.liveUrl ? (
              <a
                ...
                className="px-4 py-2 text-xs font-sans font-bold uppercase tracking-wider bg-[#1C1C1C] hover:bg-[#333333] text-[#FAFAF8] flex items-center gap-2 transition-colors shadow-sm"
              >
                <ExternalLink className="w-4 h-4" />
                Visit Live Platform
              </a>
            ) : (
              <span className="text-xs font-sans text-[#1C1C1C]/60">
          ...
          <button
            onClick={onClose}
            className="px-4 py-2 text-xs font-sans font-semibold uppercase tracking-wider bg-white hover:bg-[#FAFAF8] text-[#1C1C1C] border border-[#1C1C1C]/20 transition-colors"
          >
            Close
          </button>
        </div>

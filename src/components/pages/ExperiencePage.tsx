    ...
    <div className="pt-28 pb-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
      ...
      <div className="max-w-3xl space-y-4 pb-8 border-b border-[#1C1C1C]/15">
        <div className="inline-flex items-center gap-2 px-2.5 py-0.5 bg-[#EBE7DE] text-[#1C1C1C] text-[10px] font-sans font-bold uppercase tracking-[0.25em] border border-[#1C1C1C]/10">
          <Briefcase className="w-3 h-3 text-[#B85D19]" />
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {VERIFIED_METRICS.map((metric, idx) => (
          <div key={idx} className="p-4 bg-white border border-[#1C1C1C]/15 shadow-sm">
            <div className="text-xl sm:text-2xl font-serif italic font-bold text-[#1C1C1C]">{metric.value}</div>
      ...
      <div className="space-y-10">
        {EXPERIENCES.map((exp) => (
          <div
            key={exp.id}
            className="bg-white border border-[#1C1C1C]/15 p-6 sm:p-8 lg:p-10 shadow-sm space-y-6 relative overflow-hidden group"
          >
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-6 border-b border-[#1C1C1C]/15">
              <div className="space-y-1">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[10px] font-sans uppercase tracking-wider text-[#1C1C1C]/60 bg-[#F4F1EA] px-2.5 py-1 border border-[#1C1C1C]/10">
                    {exp.location}
                  </span>
                </div>

                <h2 className="text-2xl sm:text-3xl font-serif italic font-bold text-[#1C1C1C] tracking-tight pt-2 group-hover:text-[#B85D19] transition-colors">
                  {exp.role}
                </h2>
              </div>

              <div className="flex items-center gap-2 self-start lg:self-auto">
                <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#F4F1EA] text-[#1C1C1C] text-xs font-sans font-semibold border border-[#1C1C1C]/15">
            ...
            {/* Context Summary */}
            <div className="p-4 bg-[#F4F1EA] border border-[#1C1C1C]/10 text-xs sm:text-sm font-serif text-[#1C1C1C]/85 italic">
              <strong>Role Objective:</strong> {exp.context}
            </div>

            {/* Verified Achievements & Numerical Impacts */}
            <div className="space-y-3">
              <h3 className="text-[10px] font-sans font-bold uppercase tracking-[0.25em] text-[#B85D19] flex items-center gap-1.5">
            ...
            <div className="space-y-3">
              ...
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {exp.responsibilities.map((resp, rIdx) => (
                  <li key={rIdx} className="p-3.5 bg-[#F4F1EA] border border-[#1C1C1C]/10 flex items-start gap-2.5 text-xs sm:text-sm font-sans text-[#1C1C1C]/85 leading-relaxed">
            ...
            <div className="pt-4 border-t border-[#1C1C1C]/10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {exp.technologies.map((t, tIdx) => (
                    <span key={tIdx} className="text-xs font-sans px-2.5 py-1 bg-[#EBE7DE] text-[#1C1C1C] border border-[#1C1C1C]/10">
                      {t}
                    </span>
                  ))}
                </div>
              ...
              >
                ...
                <ArrowRight className="w-3.5 h-3.5 text-[#B85D19]" />
              </button>
            </div>
          </div>
        ))}
      ...
      <div className="p-8 bg-[#1C1C1C] text-[#F4F1EA] border border-[#1C1C1C] text-center space-y-4 shadow-sm">
        <h3 className="text-2xl font-serif italic text-[#F4F1EA]">Looking for Sanjay's Full Employment Dossier?</h3>
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
          <button
            ...
            className="px-5 py-2.5 bg-[#F4F1EA] text-[#1C1C1C] font-sans font-bold text-xs uppercase tracking-wider hover:bg-white transition-all"
          >
            Download / View Resume
          </button>
          <button
            onClick={() => onNavigate('recommendations')}
            className="px-5 py-2.5 bg-transparent text-[#F4F1EA] font-sans font-semibold text-xs uppercase tracking-wider border border-white/20 hover:bg-white/10 transition-all"
          >
